...
function! s:GrepOperator(type)
    ...
    silent execute "Ack! " . shellescape(@@) . " " . g:var_default
    copen
    redraw!
...
> ```let saved_unnamed_register = @@``` we start by declaring a variable which is set for the unnamed register
> ```
    if a:type ==# 'v'
        normal! `<v`>y
``` we start by declaring a variable which is set for the unnamed register
